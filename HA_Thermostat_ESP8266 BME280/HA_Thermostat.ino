...
void loop()
{
  ...
  if (now - LAST_SENSOR_READ > (SENSOR_UPDATE * 1000))
  {
    ...
    float c = bme.readTemperature();    // READ TEMPERATURE CELCIUS
    float h = bme.readHumidity();       // READ HUMIDITY
    float t =  c * 9/5 + 32;            // CONVERT TEMPERATURE
